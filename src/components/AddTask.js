...
const AddTask = () => {
    ...
    const updateReminder = (id,reminder)=>{
        ...
        // axios.put(`/api/updateReminder/${id}/${reminder}`).then((res)=>{console.log(res.data);});
    }
    return (
            ...
            </thead>
            {
                displayTask.map((item)=>{
                    return(
                        ...
                        <button type='submit' onClick={()=>{updateReminder(item._id,item.reminder)}}>UpdateReminder</button>
                        </tr>
                    </tbody>
                    )
                })
